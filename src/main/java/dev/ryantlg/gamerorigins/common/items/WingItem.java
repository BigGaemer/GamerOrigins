package dev.ryantlg.gamerorigins.common.items;
...
public class WingItem extends TrinketItem {
	...
	public WingItem(DyeColor primaryColour, DyeColor secondaryColour, WingType wingType) {
		super(new Item.Settings().group(Gamer.ITEM_GROUP).maxCount(1).maxDamage(Gamer.getConfig().wingsDurability).rarity(wingType == WingType.UNIQUE ? Rarity.EPIC : Rarity.RARE));
	...
	public boolean isUsable(ItemStack stack) {
		return Gamer.getConfig().wingsDurability <= 0 || stack.getDamage() < stack.getMaxDamage() - 1;
	}

	@Override
	public void tick(ItemStack stack, SlotReference slot, LivingEntity entity) {
		if(entity instanceof PlayerEntity player) {
			if(Gamer.HAS_POWERED_FLIGHT.test(player))
				return;
